...
export default function MusicEditPage() {
  ...
  // 当音频 URL 变化时自动获取时长（覆盖旧值）
  const autoDuration = useAudioDuration(audioUrl);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetchSong(controller.signal);
    return () => controller.abort();
  }, [id]);

  const fetchSong = async (signal: AbortSignal) => {
    // id 尚未就绪时跳过
    if (!id) return;

    try {
      const res = await adminFetch(API.ADMIN.MUSIC_DETAIL(id), { signal });
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-serif font-bold text-text tracking-tight">编辑歌曲</h1>
        <p className="text-sm text-text-light/70 mt-1">修改歌曲信息或重新上传文件</p>
      </div>
